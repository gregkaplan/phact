...
function vResidual = equilibriumConditions3(vars)
...
g = [gg;g_end];
inflation = vars(2*I*J,1) + varsSS(2*I*J,1);
output = vars(2*I*J+1,1) + varsSS(2*I*J+1,1);
m = vars(2*I*J+2,1) + varsSS(2*I*J+2,1);
logAggregateTFP = vars(2*I*J+3,1);
monShock = vars(2*I*J+4,1);

VDot = vars(nVars+1:nVars+I*J,1);
gDot = vars(nVars+I*J+1:nVars+2*I*J-1,1);
inflationDot = vars(nVars+2*I*J,1);
outputDot = vars(nVars+2*I*J+1,1);
logAggregateTFPDot = vars(nVars+2*I*J+3,1);
monShockDot = vars(nVars+2*I*J+4,1);

VEErrors = vars(2*nVars+1:2*nVars+I*J,1);
pcEErrors = vars(2*nVars+I*J+1,1);

logAggregateTFPShock = vars(2*nVars+nEErrors+1,1);
monShockShock = vars(2*nVars+nEErrors+2,1);
...
% Compute prices
r_nominal = rSS + pphiInflation * inflation + pphiOutput * ((output - YSS) / YSS) + monShock;
r_real = r_nominal - inflation;
rK = (r_real + ddelta) / (1 + ((1 - m) / (aalpha * m)));

% Compute aggregate output and wages
K = (reshape(aa,I*J,1) .* g)'*da_stacked;
N = (((1 - aalpha) * m * (z_avg ^ pphi) * (1 - ttau) * exp(logAggregateTFP) * (K ^ aalpha)) / cchi) ^ (1 / (pphi + aalpha));
outputHat = exp(logAggregateTFP) * (K ^ aalpha) * (N ^ (1 - aalpha));
w = (1 - aalpha) * m * (outputHat / N);
nHours = N / z_avg;
...
% NK Phillips Curve
pcResidual = r_real * inflation - ((eepsilon - 1) / ttheta) * ((eepsilon / (eepsilon - 1)) * m - 1) - ...
	(inflationDot - pcEErrors);
	
% Output
outputResidual = outputHat - output;

% Marginal costs residual
mcResidual = m - ((rK / aalpha) ^ aalpha) * ((w / (1 - aalpha)) ^ (1 - aalpha));

% Laws of motion for aggregate shocks
tfpResidual = logAggregateTFPDot - (-nnuTFP * logAggregateTFP + ssigmaTFP * logAggregateTFPShock);
monResidual = monShockDot - (-nnuMon * monShock + ssigmaMon * monShockShock);

% Package
vResidual = [hjbResidual;gResidual;pcResidual;outputResidual;mcResidual;tfpResidual;monResidual];
