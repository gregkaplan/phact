...
function [r_guess,w,output,A_st,V,g,c,dV_Upwind,dVf,dVb,If,Ib,I0] = computeSteadyState_flex()
...
for iRrho = 1 : Irrho
	...
	for ir = 1 : Ir
		...
		K = ((aalpha / (r_guess + ddelta)) ^ (1 / (1 - aalpha))) * (z_avg * nSS);
		output = (K ^ aalpha) * ((z_avg * nSS) ^ (1 - aalpha));
		w = (1 - aalpha) * output / (z_avg * nSS);
