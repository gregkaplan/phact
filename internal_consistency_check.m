function [epsilon] = internal_consistency_check(G1,impact,n_g_red,from_red,to_red,g1,F,n_v,n_g,T,steadystate,plotting,dt)
...
for i = 1:N
    p_red(:,:,i) = B_pvg*from_red(1:n_v+n_g,:)*g_red+steadystate(n_v+n_g+1:n_v+n_g+n_p);
    p(:,:,i) = B_pv*F(g)+B_pg*g+steadystate(n_v+n_g+1:n_v+n_g+n_p);
    g_red = g_red + dt*G1*g_red;
    g = g + dt*A(g);
    if i == 20
        timed = toc(tstart);
        estimated_runtime = timed*N/20/60;
