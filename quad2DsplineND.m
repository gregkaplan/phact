function [to_fine, from_fine] = quad2DsplineND(x_fine,y_fine,x_knots,y_knots)
% function [to_fine, from_fine] = quad2DsplineND(x_fine,y_fine,x_knots,y_knots,n_prior,n_post)
% Create 2D quadratic spline basis reduction
%
...
%    n_prior = number of points stacked prior to basis to finer grid points
%    n_post = number of points stacked after the dimension reduction
%
...
% Sometimes less than full basis are required, so reduce with SVD
[u,d,~] = svd(full(to_fine),'econ');
singles = find(diag(d)>(d(1,1)*10*eps));

% Basis Change Functions
to_fine = u(:,singles)*d(singles,singles);
from_fine = (d(singles,singles)\u(:,singles)');  % We took an SVD, so we can just invert by multiplication

% to_fine = kron(kron(speye(n_post),to_fine),speye(n_prior));
% from_fine = kron(kron(speye(n_post),from_fine),speye(n_prior));
