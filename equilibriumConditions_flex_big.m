...
function vResidual = equilibriumConditions_flex_big(vars)
...
K = vars(2*I*J) + varsSS(2*I*J);
r_real = vars(2*I*J+1) + varsSS(2*I*J+1);
w = vars(2*I*J+2) + varsSS(2*I*J+2);
...
K_upd = (reshape(aa,I*J,1).*g)'*da_stacked;
N_upd = (((1 - ttau) / cchi) * (z_avg ^ pphi) * (1 - aalpha) * exp(logAggregateTFP) * (K_upd ^ aalpha)) ^ (1 / (pphi + aalpha));
output_upd = exp(logAggregateTFP) * (K_upd ^ aalpha) * (N_upd ^ (1 - aalpha));
kResidual = K_upd - K;
rResidual = (aalpha * output_upd / K_upd - ddelta) - r_real;
wResidual = ((1 - aalpha) * output_upd / N_upd) - w; 

% Laws of motion for aggregate shocks
tfpResidual = logAggregateTFPDot - (-nnuTFP * logAggregateTFP + ssigmaTFP * logAggregateTFPShock);

% Package
vResidual = [hjbResidual;gResidual;kResidual;rResidual;wResidual;tfpResidual];
